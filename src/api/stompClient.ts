...
export const initStompClient = (
  ...
  stompClient = new Client({
    webSocketFactory: () => new WebSocket(`ws://3.39.135.118:8080/ws-chat?token=${token}`),
    reconnectDelay: 5000,

    onConnect: () => {
      console.log('✅ WebSocket 연결 성공');

      if (subscribedPath !== subscribePath) {
        stompClient?.subscribe(subscribePath, (msg: IMessage) => {
        ...
        subscribedPath = subscribePath;
      }
      if (onConnected) {
        onConnected();  // ✅ 연결 상태 콜백 실행
      }
    },
...
/* 메시지 전송 */
export const sendMessage = (payload: any, sendPath: string) => {
